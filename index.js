...
app.get('/api/admin/drivers/pending', authenticateToken, requireRole('ADMIN'), async (req, res) => {
    try {
        const result = await pool.query(
            `SELECT u.id, u.name, u.phone, d.vehicle_plate, d.is_approved FROM drivers d JOIN users u ON d.user_id = u.id WHERE d.is_approved = FALSE ORDER BY u.created_at DESC`
        );
